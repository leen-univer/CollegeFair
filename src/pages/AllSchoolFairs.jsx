...
const AllSchoolFairs = () => {
  ...
  const pastEvents = schoolFairs?.filter(
    (item) => new Date(item?.date) < new Date()
  );
  ...
  function customSort(a, b) {
    ...
    const timeDiffA = dateA - currentDate;
    const timeDiffB = dateB - currentDate;

    if (timeDiffA >= 0 && timeDiffB >= 0) {
      return timeDiffA - timeDiffB; // Both are upcoming events, sort by the nearest upcoming event
    } else {
      return timeDiffA < timeDiffB ? 1 : -1; // One is upcoming and the other is past, prioritize upcoming
    }
  }

  const totalEvents = [...upcomingEvents, ...pastEvents];
  console.log("totalEvents", totalEvents.sort(customSort));
